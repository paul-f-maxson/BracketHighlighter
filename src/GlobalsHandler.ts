...
export default class GlobalsHandler {
  ...
  public opactiy!: string; // NOTE: should this be "opacity"?
  ...
  public onConfigChange() {
    ...
    this.transitionTime = this.configHandler.getTransitionTimeValue();
  }
  ...
  public getLongestSymbolLength(): number {
    ...
    return longestStartSymbolLength > longestEndSymbolLength
      ? longestStartSymbolLength
      : longestEndSymbolLength;
  }

  public getLongestStartSymbolLength(): number {
    let startSymbols = this.allowedStartSymbols;
    return startSymbols.reduce(function (a, b) {
      return a.length > b.length ? a : b;
    }).length;
  }
  public getLongestEndSymbolLength(): number {
    let endSymbols = this.allowedEndSymbols;
    return endSymbols.reduce(function (a, b) {
      return a.length > b.length ? a : b;
    }).length;
  }
}
